...
class RatelimitedSttpBackend[S](rateLimiterList: List[RateLimiter],
                                ...
                                cacheFor: Duration,
                                cacheMaxCount: Long
                               )(implicit monadError: MonadError[IO]) extends SttpBackend[IO, S] with LazyLogging {
  ...
  def sendCachedRateLimited[T](request: Request[T, S])(implicit platform: Platform): IO[Response[T]] = {
    cache.getIfPresent(generateUrlId(request)) match {
      ...
      case None => this.sendRatelimited(request).map {
...
object RatelimitedSttpBackend extends LazyLogging {
  def decorateF[T](rateLimiterList: List[RateLimiter],
                         service: => IO[T])(implicit monadError: MonadError[IO]): IO[T] = {
